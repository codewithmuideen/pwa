...
export async function POST(req: NextRequest) {
  if (!process.env.GMAIL_USER || !process.env.GMAIL_APP_PASSWORD) {
    console.error("[send-login-otp] Gmail credentials not configured");
  ...
  console.log(`[send-login-otp] Sending OTP to ${user.email} for user ${user.userId}`);

  try {
    const transporter = createTransport();
    await transporter.sendMail({
      from: `"Citizens Bank" <${process.env.GMAIL_USER}>`,
      to: user.email,
    ...
    console.error("[send-login-otp] Gmail error:", err);
    return NextResponse.json(
