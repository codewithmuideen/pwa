...
export function downloadStatementPdf(input: StatementPdfInput) {
  ...
  // Header
  doc.setFontSize(20);
  doc.setTextColor(13, 90, 80);
  doc.text("Citizens Bank", margin, y);
  doc.setFontSize(9);
  doc.setTextColor(100);
  doc.text("Citizens Financial Group, Inc. | Member FDIC", margin, y + 16);

  doc.setDrawColor(20, 122, 107);
  doc.setLineWidth(2);
  ...
  doc.text(
    "This statement is for informational purposes. Citizens Financial Group, Inc. Member FDIC. Equal Housing Lender. NMLS ID# 433960.",
    margin,
    y
  );

  const fileName = `Citizens_Statement_${input.period.replace(/\s+/g, "_")}.pdf`;
  doc.save(fileName);
}
